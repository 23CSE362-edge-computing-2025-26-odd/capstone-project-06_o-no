...
def model_predict_label(model, X):
    ...
    if hasattr(model, "predict"):
        try:
            ...
            probs = model.predict(X3, verbose=0)
        if probs.ndim == 1:
            pred = int(probs[0]); conf = 1.0
        else:
            pred = int(np.argmax(probs, axis=1)[0]); conf = float(np.max(probs))
        return pred, conf
...
def edge_process(env, task, edge_res, cloud_res, ga_scheduler, metrics, scaler, rf_model, dense_model, logger, db_storage):
    with edge_res.request() as req:
        ...
        if decision == 0 and task['complexity'] < COMPLEXITY_THRESHOLD:
            pred, confidence = model_predict_label(rf_model, features_normalized)
            ...
            metrics['edge'].setdefault('inference_times', []).append(inference_time)
            ...
            logger.log_task_completion(env.now, 'edge', task['id'], total_latency, energy_consumed, accuracy, pred != 0, infer_ms=inference_time)
        else:
...
def cloud_process(env, task, cloud_res, metrics, scaler, dense_model, logger, db_storage):
    with cloud_res.request() as req:
        ...
        pred, confidence = model_predict_label(dense_model, features_normalized)
        ...
        metrics['cloud'].setdefault('inference_times', []).append(processing_time)
        ...
        logger.log_task_completion(env.now, 'cloud', task['id'], total_latency, energy_consumed, accuracy, pred != 0, infer_ms=processing_time)
...
def sensor_process(env, edge_res, cloud_res, ga_scheduler, metrics, scaler, rf_model, dense_model, logger, db_storage):
    ...
    while True:
        ...
        task = {'id': task_id, 'data': data, 'arrival': env.now, 'complexity': random.uniform(0.5, 3), 'true_label': int(data['Fault_Label'].iloc[0])}
