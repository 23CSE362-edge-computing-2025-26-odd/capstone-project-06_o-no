...
class EnhancedGA:
    ...
    def schedule(self, task_complexity, edge_load, cloud_queue=0, net_latency=5):
        ...
        for _ in range(self.generations):
            ...
            new_population = [population[idx1] if fitness_scores[idx1] < fitness_scores[idx2] else population[idx2]
                              for idx1, idx2 in [np.random.choice(self.population_size, 2, replace=False) for _ in range(self.population_size)]]
            for i in range(0, self.population_size - 1, 2):
                if np.random.random() < 0.7:
                    new_population[i], new_population[i+1] = new_population[i+1], new_population[i]
        ...
        decision = int(population[np.argmin(final_fitness)])
